...
uniform float uMouseInfluence,uHoverAmount,uHoverScale,uParallax,uBurst,uCoverageAlpha;
...
float ring(vec2 p,float ri,float cut,float t0,float px){
  float t=mod(uTime+t0,CYCLE),r=ri+t/CYCLE*uScaleRate;
  float d=abs(length(p)-r),a=atan(abs(p.y),abs(p.x))/HP;
...
void main(){
  ...
  p/=sc;
  vec3 c=vec3(0.0);float coverage=0.0;
  ...
  for(int i=0;i<10;i++){
    ...
    float ra=ring(pr,uBaseRadius+fi*uRadiusStep,pow(uRingGap,fi),i==0?0.0:2.95*fi,px);
    c=mix(c,rc,vec3(ra));coverage=max(coverage,ra);
  }
  c*=1.0+uBurst*2.0;
  float n=fract(sin(dot(gl_FragCoord.xy+uTime*100.0,vec2(12.9898,78.233)))*43758.5453);
  c+=(n-0.5)*uNoiseAmount;
  float intensity=max(c.r,max(c.g,c.b));
  vec3 ec=intensity>0.0001?clamp(c/intensity,0.0,1.0):vec3(0.0);
  vec3 oc=mix(ec,clamp(c,0.0,1.0),uCoverageAlpha);
  float oa=mix(intensity,coverage,uCoverageAlpha);
  gl_FragColor=vec4(oc,clamp(oa*uOpacity,0.0,1.0));
}`;

interface MagicRingsProps {
  color?:string; colorTwo?:string; ringCount?:number; speed?:number;
  attenuation?:number; lineThickness?:number; baseRadius?:number;
  radiusStep?:number; scaleRate?:number; opacity?:number; blur?:number;
  ...
  clickBurst?:boolean; alphaMode?:"luminance"|"coverage";
}

export default function MagicRings({
  color="#119822", colorTwo="#31cb00", ringCount=6, speed=1,
  ...
  propsRef.current={color,colorTwo,speed,ringCount,attenuation,lineThickness,baseRadius,radiusStep,scaleRate,opacity,noiseAmount,rotation,ringGap,fadeIn,fadeOut,followMouse,mouseInfluence,hoverScale,parallax,clickBurst,alphaMode};
  ...
  useEffect(()=>{
    ...
    const uniforms={
      ...
      uHoverAmount:{value:0},uHoverScale:{value:1},uParallax:{value:0},
      uBurst:{value:0},uCoverageAlpha:{value:0}
    };
    ...
    let frameId=0,isVisible=false,isPageVisible=!document.hidden,elapsed=0,lastT=0;
    const animate=(t:number)=>{
      frameId=requestAnimationFrame(animate);
      const p=propsRef.current as Required<MagicRingsProps>;
      const dt=lastT===0?0:Math.min(t-lastT,100); lastT=t;
      ...
      uniforms.uColor.value.set(p.color??"#119822");
      uniforms.uColorTwo.value.set(p.colorTwo??"#31cb00");
    ...
    const tryStart=()=>{if(isVisible&&isPageVisible&&frameId===0){lastT=0;frameId=requestAnimationFrame(animate);}};
    ...
    tryStart();
    return()=>{
      tryStop(); io.disconnect(); window.removeEventListener("resize",resize); ro.disconnect();
