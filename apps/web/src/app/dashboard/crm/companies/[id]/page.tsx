...
import { ArrowLeft, Globe, MapPin, Calendar, FileText, CheckSquare, MoreVertical, Star, Edit2, Trash2, Mail, Phone, ExternalLink } from "lucide-react";
...
export default function CompanyDetailPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [companyRes, activityRes] = await Promise.all([
        fetch(`/api/crm/companies`, { headers: { 'x-org-id': orgId || '' } }),
        // fetch(`/api/crm/activities?companyId=${companyId}`, { headers: { 'x-org-id': orgId || '' } }) // if backend supported companyId filter
        // Actually the backend endpoint only filters by contactId right now...
        // Wait, I didn't update the backend to filter by companyId in GET /api/crm/activities. I will need to handle that, but for now I'll just fetch all and filter client side for safety, or I should update backend.
        // Let's assume I'll update backend soon, but for now let's just make the request.
      ...
      if (companyRes.ok) {
        ...
        if (found) {
          setCompany(found);
          setFormData({
            name: found.name, domain: found.domain || "", industry: found.industry || "", address: found.address || ""
          });
        }
      }
      ...
      if (activityRes.ok) {
        ...
        // filter by companyId client-side temporarily until backend is updated
        const filtered = (data.activities || []).filter((a: any) => a.companyId === companyId);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/crm/companies/${company.id}`, {
      ...
      body: JSON.stringify({ ...formData, isStarred: company.isStarred }),
    });
    if (res.ok) {
      setIsEditing(false);
      fetchData();
  ...
  const toggleStar = async () => {
    if (!company) return;
    const orgId = localStorage.getItem('cordibase_active_org');
    setCompany({ ...company, isStarred: !company.isStarred });
  ...
  if (!company) {
    return <div className="p-12 text-center text-ink/60">Company not found.</div>;
  }
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={toggleStar} className="p-1 hover:bg-linen rounded-md transition-colors">
                <Star className={`w-5 h-5 ${company.isStarred ? 'fill-[#F59E0B] text-[#F59E0B]' : 'text-[#D0D5DD]'}`} />
              </button>
            </div>
            <p className="text-sm text-ink/60">{company.industry || "No industry specified"}</p>
          </div>
        </div>
            ...
            <>
              ...
              <div className="absolute right-0 top-10 w-48 bg-white border border-ink/10 shadow-modal rounded-lg py-1 z-20 animate-in zoom-in-95">
                <button onClick={() => { setIsEditing(true); setShowOptions(false); }} className="w-full text-left px-4 py-2 text-sm text-ink hover:bg-linen flex items-center gap-2">
                  <Edit2 className="w-4 h-4 text-ink/60" /> Edit Company
                </button>
                ...
                <button onClick={() => { handleDelete(); setShowOptions(false); }} className="w-full text-left px-4 py-2 text-sm text-[#DC2626] hover:bg-[#FEF2F2] flex items-center gap-2">
                  <Trash2 className="w-4 h-4 text-[#DC2626]" /> Delete Company
                </button>
              </div>
            </>
          )}
        ...
        <div className="w-80 flex-shrink-0 space-y-4">
          <div className="bg-white border border-ink/10 rounded-xl p-5">
            ...
            <div className="space-y-4 text-sm">
              {company.domain && (
                <div>
                  <div className="text-ink/60 mb-1 flex items-center gap-2"><Globe className="w-4 h-4"/> Website</div>
                  <a href={`https://${company.domain.replace(/^https?:\/\//, '')}`} target="_blank" rel="noopener noreferrer" className="text-thread hover:underline font-medium flex items-center gap-1">
              ...
              {company.address && (
                <div>
                  <div className="text-ink/60 mb-1 flex items-center gap-2"><MapPin className="w-4 h-4"/> Address</div>
                  <div className="text-ink font-medium leading-relaxed">{company.address}</div>
                </div>
              )}
              {!company.domain && !company.address && (
                <div className="text-ink/60 italic">No company details provided.</div>
              )}
            </div>
            
            <div className="mt-6 pt-6 border-t border-ink/10">
              <a href={company.domain ? `https://${company.domain.replace(/^https?:\/\//, '')}` : '#'} target="_blank" rel="noopener noreferrer" className={`w-full py-2 bg-white border border-ink/10 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 ${company.domain ? 'hover:bg-linen text-ink' : 'opacity-50 cursor-not-allowed text-ink/60'}`}>
        ...
        <div className="fixed inset-0 bg-[#1D2939]/50 flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-xl shadow-modal w-full max-w-md overflow-hidden animate-in zoom-in-95">
            <div className="p-4 border-b border-ink/10 flex justify-between items-center">
              <h3 className="font-semibold text-ink">Edit Company</h3>
